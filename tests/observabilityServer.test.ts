...
describe("ObservabilityHttpServer", () => {
  ...
  it("serves health, readiness, and metrics without legacy dashboard routes", async () => {
    let ready = false;
    const config = {
    ...
    expect((await fetchText(baseUrl, "/dashboard")).status).toBe(404);
    expect((await fetchText(baseUrl, "/api/tasks/recent?limit=10", "secret")).status).toBe(404);
    expect((await fetchText(baseUrl, "/api/metrics/summary", "secret")).status).toBe(404);
  });
