...
| `PROJECT_MANAGER_RUN_ONCE` | Нет | `false` | Запускает один PM analysis cycle вместо periodic schedule, когда agent включен. |
| `PROJECT_MANAGER_INTERVAL_MINUTES` | Нет | `1440` | Интервал между scheduled PM analysis cycles. |
...
| `PROJECT_MANAGER_RUN_ONCE` | `false` | Runs one analysis cycle instead of a periodic schedule when PM is enabled. |
| `PROJECT_MANAGER_INTERVAL_MINUTES` | `1440` | Periodic analysis interval. |
